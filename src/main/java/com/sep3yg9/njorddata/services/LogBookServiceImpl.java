...
@Service public class LogBookServiceImpl implements LogBookService {
    ...
    public LogBookServiceImpl(ProjectRepository projectRepository, MeetingRepository meetingRepository, LogbookEntityRepository logBookEntityRepo, LogbookentryEntityRepository logBookEntryEntityRepo) {
    ...
    @Override public void updateLogBook(UpdatingLogBook logBook) {
        ...
        for(LogBookEntryGrpc entry : logBook.getLogbookentriesList()){
            MeetingEntity meeting = meetingRepository.findByIdmeeting(entry.getAssignedmeeting());
            LogbookentryEntity entity = new LogbookentryEntity(logbookEntity, meeting, entry.getContents());
